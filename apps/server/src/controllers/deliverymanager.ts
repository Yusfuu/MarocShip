...
const login = catchAsync(async (req: Request, res: Response) => {
  ...
  //@ts-ignore
  deiveryManager.role = "DELIVERYMANAGER";

  //@ts-ignore
  req.session.user = deiveryManager;
  await req.session.save();

  res.status(200).json({ payload: deiveryManager });
});
...
const create = catchAsync(async (req: Request, res: Response) => {
  ...
  const driverToken = await seal({ _id: delivery._id });
